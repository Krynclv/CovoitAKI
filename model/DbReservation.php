...
class DbReservation{
    ...
    public static function getListePassager($email)
    {
        $sql = "SELECT * FROM `utilisateurs`, reservation WHERE utilisateurs.iduser = reservation.iduser AND reservation.Valide =1;";		
		$objResultat = connectPdo::getObjPdo()->query($sql);	
		$result = $objResultat->fetchAll();
		return $result;
